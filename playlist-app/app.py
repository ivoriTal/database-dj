...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:seth06568@localhost/playlist-app'
...
def add_playlist():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"Added {new_playlist.name} playlist.", "success")
        return redirect("/playlists")
...
def add_song():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"Added {new_song.title} by {new_song.artist}.", "success")
